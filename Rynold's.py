...
class RynoldNumber(object):
    def __init__(self,Gas:str,u_infinity:float,length,pressure:float=1*10**5,temp:float=100):
        """
        ...
        self.Value=0
        try:
            # Trying to get the dynamic viscosity directly without interpolation
            
            self.Vis=self.pd[(self.pd['temp']==temp)]['Vis']
        except:
            # getting the viscosity by newton interpolation
            Viscosity=Interp(Xaxis=self.pd['temp'].to_list(),Yaxis=self.pd['kyn_vis'].to_list())
            self.Vis=Viscosity.newton_interpol(temp)
